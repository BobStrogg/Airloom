...
function parseHostBind(value: string | undefined): string {
  const bind = value?.trim() || DEFAULT_HOST_BIND;
...
export function parseHostEnv(cliPort?: number): HostEnvConfig {
  ...
  const hostBind = parseHostBind(process.env.HOST_BIND);
